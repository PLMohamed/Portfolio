...
import { CreateToken, DeleteTokenById, GetTokensByUserId } from "@/db/queries";
...
export async function getSessionToken(): Promise<UserTokenPayload | null> {
  try {
    ...
    const tokenCookie = cookieStore.get(COOKIE_NAMES.TOKEN);
    ...
    const storedToken = await GetTokensByUserId(payload.uuid);

    if (!storedToken.length || !storedToken.some((t) => t.token === token)) {
      after(() => {
        storedToken.forEach((t) => {
          revokeSessionToken(t.id);
        });
      });
 ...
 * ```
 */
export async function revokeSessionToken(tokenId: string): Promise<boolean> {
  try {
    const result = await DeleteTokenById(tokenId);
    return !!result;
  } catch (error) {
...
export async function revokeAllUserTokens(userId: string): Promise<number> {
  try {
    ...
    const result = await Promise.all(
      tokens.map((token) => revokeSessionToken(token.id)),
    );
